...
class FunctionTimedOut(BaseException):
    ...
    def getMsg(self):
        ...
        return "Function %s (args=%s) (kwargs=%s) timed out after %s seconds.\n" % (
            timedOutFuncName,
            repr(self.timedOutArgs),
            repr(self.timedOutKwargs),
            timedOutAfterStr,
        )

    @classmethod
    def create_exception_type(cls, timeout, func, *args, **kwargs) -> type:
        class FunctionTimedOutTempType(cls):
            def __init__(self):
                return cls.__init__("", timeout, func, args, kwargs)

        exception_name = (
            "FunctionTimedOut"
            + str(hash("%d_%d_%d_%d" % (id(timeout), id(func), id(args), id(kwargs)))),
        )
        return type(
                exception_name,
